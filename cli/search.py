...
@click.pass_context
def search_cmd(ctx, query, top_k, refine, no_refine, interactive, max_content, show_refinement):
...
@click.command()
@click.option('--limit', '-l', type=int, default=20, help='Number of recent searches to show')
@click.pass_context
def search_history(ctx, limit):
...
# Add subcommands to the search group
search_cmd.add_command(search_history, name='history')
