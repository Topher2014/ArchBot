...
@click.pass_context
def build_cmd(ctx, input, output, embedding_model, batch_size, force):
...
@click.pass_context
def build_stats(ctx):
   """Show index building statistics."""
   config = ctx.obj['config']
...
# Add subcommands to the build group
build_cmd.add_command(build_stats, name='stats')
