...
@click.pass_context
def scrape_cmd(ctx, output, delay_min, delay_max, max_retries, resume, force):
   ...
   try:
       ...
       session_data.update({
           ...
           'total_pages': success_count,  # This will be updated with actual stats
           'success_count': success_count,
           'error_count': 0,  # Will be updated with actual stats
           'skip_count': 0,   # Will be updated with actual stats
           'status': 'completed'
       })
...
@click.command()
@click.option('--limit', '-l', type=int, default=10, help='Number of recent sessions to show')
@click.pass_context
def scrape_history(ctx, limit):
...
# Add subcommands to the scrape group
scrape_cmd.add_command(scrape_history, name='history')
